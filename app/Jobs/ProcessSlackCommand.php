    ...
    public function handle(): void
    {
        ...
        try {
            ...
            foreach ($urls as $url) {
                ...
                if (in_array($response->status(), [200, 201])) {
                    ...
                    if (in_array($response->status(), [200, 201])) {
                        ...
                        $slackRes = Http::withToken($token)->post('https://slack.com/api/conversations.invite', $payload);
                        // dd($slackRes);
            ...
            if ($response->object()->name !== $this->dogs[$nameIndex]) {
                ...
                $message = "After maing an update request, I can't find {$this->names[$nameIndex]}.  Please fixed up";
